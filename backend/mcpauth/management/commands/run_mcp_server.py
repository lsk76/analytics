...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        from mcpauth.policy import ALL_SCOPES, SCOPE_READ
        ...
        server = MCPServer(
            ...
            instructions=(
                ...
                '`task_update`. Перетегувати вже зібрані події — `posts_retag` '
                '(стеля 25, спершу confirm=false). Пости дивись `posts_list` / `post_show`. '
                'Повернути конкретні пости в чергу — `posts_requeue` (спершу confirm=false; '
                'пости, що вже на події, лише з drop_events=true). Повний скид усіх постів у чергу скріну '
                'сюди не винесено: він видаляє події. Глобальні key-value — `settings_list`, '
                'повний текст — `setting_show`, запис — `setting_set`.\n\n'
            ...
            auth=AuthSettings(
                ...
                # Реєструємо клієнта з МІНІМУМОМ (`mcp:read`): у запиті клієнта
                # не має світитися `mcp:admin`. Скоупи видає роль — див.
                # policy.granted_scopes (клієнт, що не звужував запит свідомо,
                # дістає все, що дає його роль).

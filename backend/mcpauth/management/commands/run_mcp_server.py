...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        server = MCPServer(
            name="tg-analytics",
            instructions=(
                'Керування сервісом аналітики Telegram: акаунти й проксі, моніторинги (задачі/чати/джерела), збори, черги конвеєрів, TeleZip, публікація.'
                ''
                'Починай зі `service_health` — він показує, що стоїть і хто це розгрібає.'
                ''
                'TeleZip — це три ендпоінти: `tz_find` (пошук повідомлень; `stats=true` віддає'
                'лічильники без викачування), `tz_channels` (пошук каналів), `tz_users`'
                '(профілі людей). УВАГА: кожен виклик до TeleZip коштує ~$0.10 незалежно від'
                'обсягу, тому обсяг питай `tz_find(stats=true)`, а не пошуком навмання.'
                'У запитах TeleZip ПРОБІЛ означає АБО, а не І: для «і» потрібен `+`.'
                ''
                'Інструменти з поміткою «ЗМІНЮЄ СТАН» пишуть у БД або чіпають контейнери.'

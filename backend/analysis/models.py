...
class PublishConfig(models.Model):
    ...
    stop_patterns = models.TextField(
        blank=True, default="", verbose_name="Стоп-фрази (регулярки, по одній на рядок)",
        help_text="Подія НЕ публікується, якщо будь-яка регулярка збігається з текстом "
                  "поста або з резюме події. Для правил, які модель ігнорує попри промпт: "
                  "у виборчому моніторі так відсіювали «Путін проголосував» — замовник "
                  "ставив 👎, а скрін уперто давав важливість 3. Порожній рядок і рядок, "
                  "що починається з #, пропускаються.")

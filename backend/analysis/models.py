...
class PublishConfig(models.Model):
    ...
    raw_mode = models.BooleanField(
        ...
        help_text="Публікувати ОРИГІНАЛЬНИЙ текст повідомлення/поста замість переказу "
                  "від ШІ — нуль спотворень. Рерайту немає, але AI-промпт нижче "
                  "працює: якщо він заповнений, ШІ вирішує ЧИ публікувати (фільтр), "
                  "а в канал усе одно йде текст першоджерела. Порожній AI-промпт = "
                  "LLM не викликається зовсім.",
    )

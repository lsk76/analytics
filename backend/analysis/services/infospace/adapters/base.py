...
Правила контракту (закріплені контракт-тестом по реєстру ADAPTERS):
  ...
  2. Повертає ЛИШЕ НОВІ елементи відносно source.state (watermark свого kind);
     source.state мутується В ПАМ'ЯТІ, зберігає його стадія ПІСЛЯ успішного
     upsert постів (щоб збій не загубив елементи).
  3. Мережеві/парс-помилки → виняток; backoff і health рахує стадія.
  4. Ліміти: config["max_items"] на виклик; перший полінг (порожній state) —

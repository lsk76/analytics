...
## Оператори в query/exact/channel_term — УВАГА, ЦЕ v4
АБО = `|`   І = пробіл МІЖ ГРУПАМИ   `-слово` = НЕ (пишеться ЗЛИТНО)
`( … )` групування · `слово*` префікс · `"фраза"` · `"a b"~N` відстань N слів
Робоча форма: (тема | тема2) (дія | дія2)
  (мигрант | диаспора) (драка | избил | напал)

‼️ У v3 (`/Find` — конвеєр, `task.telezip_query`) ВСЕ НАВПАКИ: пробіл = АБО,
`+` = І. Ці інструменти ходять у v4. Запит у чужому діалекті НЕ помиляється —
він тихо віддає 0, і це не відрізнити від «немає згадок»: `чернавин белокопытов
саханефтегазсбыт` у v4 = І трьох слів = нуль. Тому НЕ копіюй запит задачі сюди
без перекладу (і навпаки, не неси звідси запит у task_update як є).
Двобуквені токени в групі тягнуть чужомовний шум: `ер` ловить узбецьке й
казахське «ер» — пиши `единоросс* | едрос* | едро`.

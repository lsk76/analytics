...
_EXACT = ('ДОСЛІВНО, без відмінків — для абревіатур, марок, серійних номерів: '
          'exact="сво" не зачепить "свой"/"своего", а query="сво" зачепить. '
          'Синтаксис операторів той самий, що й у query. Можна разом із query: '
          'тоді умови комбінуються (exact="сво" + query="бригада").')
...
_CHTERM = ('Фільтр ПО КАНАЛУ, а не по повідомленню: спершу відбираються канали, '
           ...
           'Синтаксис як у query. Приклад: channel_term="крым крим АРК". '
           'Не став багато поширених слів — понад 20 000 каналів = відлуп.')
...
_EXTRA = ('JSON із додатковими полями тіла запиту — для того, що з\'явилось в API '

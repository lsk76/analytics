...
Скоупи видає РОЛЬ, а не запит клієнта: Claude реєструється з мінімальним
`mcp:read` (в його запиті не має світитися `mcp:admin`) і повторює цей рядок
при авторизації — це не «прошу лише читання», тож стелю воно не зрізає
(`policy.granted_scopes`). Свідомо ВУЖЧИЙ за виданий запит поважається.

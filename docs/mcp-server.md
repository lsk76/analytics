...
| `operator` | `+ mcp:write` | збори, правки чатів/джерел/акаунтів/подій — того, що вже є |
...
третій шар, і він теж ОДИН на систему: `analysis/services/access.py` (`RULES`
— правило на модель), звідки його беруть і адмінка (`ScopedAdminMixin`), і MCP
(`mcp_api.common.scope`). Тест `test_access_rules.py` звіряє, що для кожної
моделі адмінка й MCP віддають однакові рядки, і не дає ModelAdmin завести
власний фільтр видимості. Групи з міграцій: «Telegram-акаунти»

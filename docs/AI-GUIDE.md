...
**КАНОН з 2026-07-07 (зафіксовано після A/B-серії на 300 коментарях):**
- промпти живуть у ПОЛЯХ ЗАДАЧІ (prescreen_prompt/tagger_prompt, адмінка);
  prompts.py — лише запасний варіант;
- обидва промпти АНГЛІЙСЬКОЮ, ключі тегів українською дослівно; осі opinion НЕМАЄ;
- тегувальник v4 «ворота→теги»: СПОЧАТКУ validation gate (адресат-влада? власна
  оцінка автора? не побут/не сварка/не іноземці/не чат-адмін?), і лише потім теги
  за визначеннями (по рядку на кожен із 19 тегів) + старшинство (безіменні
  «власти» в чаті республіки = крит_рег_правит; крит_кремль лише з явним фед.
  маркером). Дає 41.7% від кандидатів, рег_правит домінує;
- ВИКОНАВЕЦЬ ТЕГУВАННЯ: ЛИШЕ Haiku-агенти (gemini на v4 ігнорує ворота і
  старшинство: 51% і плоска розкладка; безкоштовні OpenRouter — вічні 429).
  ВИКОНАВЦЯ МІЖ ПЕРІОДАМИ НЕ МІНЯТИ — інакше динаміка ламається;
- окремого скептика-валідатора НЕ використовуємо (рішення 2026-07-07): ворота
  v4 дають ~60% його ефекту безкоштовно;
- прескрін: gemini-flash, англ. compact-промпт (10.3% позитивів, збіг зі старим 95%).
Деталі: **docs/comments-analysis-pipeline.md**.
Разовий перенос історії: `_dir/materialize_comment_events.py` (ідемпотентний).

### ⚠️ ДОСЛІД «кампанія вирівнювання, лютий» — ПЕРЕРОБИТИ (стан 2026-07-08)

Пілот вирівнювання історії критики (задача id=9 `criticism-canon-v4`, запуск #30,
лютий 2026) ЗУПИНЕНО користувачем на ~131/946 пачок: повний потік виявився
завеликим для агентного тегування (47к кандидатів/міс ≈ 946 пачок ≈ 16М токенів).
Сам збір і канон v4 валідні (Telethon підтвердив обсяги: TG ≥ TeleZip по всіх
доступних чатах; мегачат — 34 млн повідомлень за життя, історія прихована).

ПЕРЕРОБКА (підхід погодити з користувачем перед стартом):
  * історія (вер.2025–чер.2026) — ВИБІРКОЮ: кожна 3-тя пачка + поріг впевненості
    прескріну ≥0.6 (~200 пачок/міс; частки ±2 в.п.; абсолюти = оцінка ×3);
  * поточні місяці (з липня 2026) — повний потік щоденними порціями;
  * вже відтеговані 131 пачок лютого (v4) — перевикористати, вони валідні.
Запуск #30 стоїть у awaiting_agent — безпечно; перед переробкою або дотегнути
до вибіркової сітки, або перестворити запуск під нову сітку пачок.
Граблі агентних хвиль: вимагати «РІВНО стільки items, скільки у вхідному» і
«НЕ запускай інших агентів»; після кожної хвилі — скрипт-звірка повноти
(неповні done-файли видалити й перетегувати).
...
- **TeleZip має ГЛИБИНУ індексу** (`/v4/stats` → `searchDateLimit`; 2026-09-18 —
  ...
  **docs/telezip-api.md**, з чату — `tz_find` / `tz_channels` / `tz_users`
  (по одному на ендпоінт; `tz_find(stats=true)` — обсяг без викачування).

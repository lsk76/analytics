...
> **Статус: Phase 0-3 РЕАЛІЗОВАНО.** Дата: 2026-07-09. Три адаптери працюють
> наживо: RSS (`gazeta-n1.ru`→5 подій), Web-скрапінг (`baikal-daily.ru`,
> discovery+trafilatura), Telegram (`ulan_smi` акаунтом). 74 тести зелені.
> Phase 0+1 закомічено (гілка `infospace-pipeline`); Phase 2+3 — далі.
> Первісний дизайн нижче.
